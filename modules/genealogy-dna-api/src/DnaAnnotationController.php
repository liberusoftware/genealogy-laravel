...
final class DnaAnnotationController
{
    ...
    {
        $record = $create->execute($request->validate(['noteable_type' => ['required', 'string'], 'noteable_id' => ['required', 'uuid'], 'body' => ['required', 'string', 'max:50000']]));
    ...
    {
        $values = $request->validate(['page' => ['sometimes', 'array'], 'page.size' => ['sometimes', 'integer', 'between:1,100'], 'match_id' => ['sometimes', 'uuid'], 'person_id' => ['sometimes', 'uuid']]);
    ...
    {
        $record = $create->execute($request->validate(['match_id' => ['required', 'uuid'], 'person_id' => ['required', 'uuid'], 'relationship_type' => ['required', 'string', 'max:100'], 'confidence' => ['nullable', 'integer', 'between:0,100'], 'status' => ['sometimes', 'in:proposed,confirmed,rejected'], 'rationale' => ['nullable', 'string', 'max:10000'], 'metadata' => ['nullable', 'array']]));
