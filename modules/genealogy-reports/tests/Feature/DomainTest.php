...
it('creates and hydrates its aggregate through the domain action', function (): void {
    ...
    $database->schema()->create('genealogy_reports', function ($table): void {
    ...
    $record = (new CreateGenealogyReport())->execute([
        'name' => 'Sample record',
        'status' => 'active',
        'metadata' => ['source' => 'archive'],
    ]);

    expect($record)->toBeInstanceOf(GenealogyReport::class)
