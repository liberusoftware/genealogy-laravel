...
it('creates and hydrates its aggregate through the domain action', function (): void {
    ...
    $database->schema()->create('genealogy_data_transfers', function ($table): void {
    ...
    $record = (new CreateDataTransfer())->execute([
        'name' => 'Sample record',
        'status' => 'active',
        'metadata' => ['source' => 'archive'],
    ]);

    expect($record)->toBeInstanceOf(DataTransfer::class)
